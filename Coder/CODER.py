    ...
    def get_name_texts(self):
        ...
        with torch.no_grad():
            for class_name in tqdm(self.class_name_lists):
                ...
                text_inputs = clip.tokenize(get_templates(self.dataset, 'name', class_name=class_name),
                                            truncate=True).cuda()
        ...
        with torch.no_grad():
            for i in tqdm(range(len(attribute))):
                ...
                for j in range(len(attribute[i])):
                    input_list = input_list + get_templates(self.dataset, "att", class_name=class_name,
                                                            att=attribute[i][j], task_type=self.task_type)
        ...
        with torch.no_grad():
            for i in tqdm(range(len(analogous_classes))):
                ...
                for j in range(len(analogous_classes[i])):
                    input_list = input_list + get_templates(self.dataset, "ana", class_name=class_name,
                                                            ana=analogous_classes[i][j], task_type=self.task_type)
                for input in input_list:
